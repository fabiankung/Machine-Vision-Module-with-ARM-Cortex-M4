...
unsigned int	gunMaxLuminance;		//
unsigned int	gunMinLuminance;		//
unsigned int	gunAverageLumROI;		// For debug purpose, average luminance value within ROI.
	...
	if (ptrTask->nTimer == 0)
	{
		switch (ptrTask->nState)
		{
			...
			if (gnFrameCounter != nCurrentFrame)		// Check if new image frame has been captured.
			{
				...
				nymax[0] = gnImageHeight>>1;			// to center of the frame.
...
#define		__IP2_ROIX_START	40		// Start position in pixels for ROI (upper left corner).
#define		__IP2_ROIY_START	93	
#define		__IP2_ROI_WIDTH		27		// The width of a single ROI region in pixels.
#define		__IP2_ROI_WIDTH_TOTAL	3*__IP2_ROI_WIDTH	// Total ROI width
#define		__IP2_ROI_HEIGHT	8		// The height of a single ROI region in pixels.
#define		__IP2_ROI_HEIGHT_TOTAL	3*__IP2_ROI_HEIGHT
...
void Proce_Image2(TASK_ATTRIBUTE *ptrTask)
{
	...
	static unsigned int	unLumCumulativeROI;
	static unsigned int unAverageLumROI;
	static int	nPixelCount;
	...
	static int nROIXindex, nROIYindex;
	...
	if (ptrTask->nTimer == 0)
	{
		switch (ptrTask->nState)
		{
			case 0: // State 0 - Wait for the correct process ID, then start initialization.
				if ((gnImageProcessingAlgorithm == 2) && (gnCameraReady == _CAMERA_READY))
				{
					...
					gunAverageLumROI = unAverageLumROI;		// For debug purpose.														
					gnEyeLED1 = 1;
					gnEyeLED2 = 1;
					gnEyeLEDEffect = _EYE_LED_EFFECT_NONE;	
			...
			case 1: // State 1 - Wait until a new frame is acquired in the image buffer, initialize de-noising.
				...
				{
					nCurrentFrame = gnFrameCounter;							// Update current frame counter.
					nYindex = __IP2_ROIY_START-1;							// Set the y start position for the de-noising process.
					nROIXOffset =  __IP2_ROIX_START_OFFSET;										
																			...
																			// the camera is not pointing straight ahead eventhough
			...
			case 2: // State 2 - Perform morphology operation on image frame, one row at a time in the ROI until all rows in ROI 
					// are processed.
				PIOA->PIO_ODSR |= PIO_ODSR_P19;								// Set flag, this is optional, for debugging purpose.
				
				for (nXindex = __IP2_ROIX_START+__IP2_ROIX_START_OFFSET; nXindex < (__IP2_ROIX_START+__IP2_ROIX_START_OFFSET+__IP2_ROI_WIDTH_TOTAL+1); nXindex++)							
				{
					...
					{
						nLuminance = nLumReference;							// White.
						nLBP = 1;											// Set bit0.
						nCounter++;
					}
				...
				nYindex++;
				if (nYindex == __IP2_ROIY_START+__IP2_ROI_HEIGHT_TOTAL)		// Is it last row?
				{															// Yes
					nROIXindex = 0;											// Initialize the ROI index.
					nROIYindex = 0;
					OSSetTaskContext(ptrTask, 3, 1);						// Next state = 3, timer = 1.
					nROIStartX = __IP2_ROIX_START + nROIXOffset;			// Set the start pixel and size of each sub-ROI.
					nROIStartY =__IP2_ROIY_START;
					nROIWidth = __IP2_ROI_WIDTH;
					nROIHeight = __IP2_ROI_HEIGHT;
			...
			case 3: // State 3 - Check if any object detected (e.g. luminance is 0) within each sub-ROI.
	
				// ROI11 to ROI13.
				nROIThreshold[nROIXindex][nROIYindex] = 0;
				for (nYindex = nROIStartY; nYindex < (nROIStartY + nROIHeight); nYindex++)	
				{
					for (nXindex = nROIStartX; nXindex < (nROIStartX + nROIWidth); nXindex++)
					{
						if ((gunImgAtt[nXindex][nYindex] & _LUMINANCE_MASK) < nLumReference)	// Check for 'dark spots' in ROI.
						{
							nROIThreshold[nROIXindex][nROIYindex]++;	// Update the counter for 'dark spots'.	
						}
					}
				}
				...
				if (nROIXindex < 3)
				{
					nROIStartX = nXindex+1;									// Update next sub-ROIstart position for X.
					OSSetTaskContext(ptrTask, 3, 1);							// Next state = 3, timer = 1.
				}
				else
				{
					nROIXindex = 0;
					nROIYindex++;			
					nROIStartX = __IP2_ROIX_START + nROIXOffset;				// Update next sub-ROIstart position for X.
					nROIStartY = nROIStartY + nROIHeight;						// Update next sub-ROIstart position for Y.
					OSSetTaskContext(ptrTask, 4, 1);							// Next state = 4, timer = 1.
				}			
				break;
			...
			case 4: // State 4 - Check if any object detected (e.g. luminance is 0) within each sub-ROI.
				...
				for (nYindex = nROIStartY; nYindex < (nROIStartY + nROIHeight); nYindex++)
				{
					for (nXindex = nROIStartX; nXindex < (nROIStartX + nROIWidth); nXindex++)
					{
						if ((gunImgAtt[nXindex][nYindex] & _LUMINANCE_MASK) < unAverageLumROI)	// Check for 'dark spots' in ROI.
						{
							nROIThreshold[nROIXindex][nROIYindex]++;	// Update the counter for 'dark spots'.	
						}
					}
			...
			case 5: // State 5 - Check if any object detected (e.g. luminance is 0) within each sub-ROI.
			...
			case 6: // State 6 - Check if any object detected (e.g. luminance is 0) within the ROIs, report to remote controller.
				...
				gobjRec1.nHeight = __IP2_ROI_HEIGHT*3;		// Mark the ROI.
				gobjRec1.nWidth = __IP2_ROI_WIDTH*3;
				gobjRec1.nX = __IP2_ROIX_START + nROIXOffset;
				gobjRec1.nY = __IP2_ROIY_START;
